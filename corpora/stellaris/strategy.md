...
native AI executes it. The companion mod (planned, see `README.md`) applies a directive by:
setting a country flag (`governor_directive_<name>`) that `ai_weight` modifiers in the mod's
overrides read [doc:ai_modding]; toggling edicts the empire can already afford; and, rarely,
changing a policy (each change locks that policy for 10 years [doc:policies]). A directive
**never** adds resources, modifiers or anything the empire could not do itself.
...
**Observer-mode alternative (fallback, not the plan)**: the console command `observe` hands
the player empire to the AI when time runs, and `play <id>` takes it back [doc:console_commands].
It needs no mod but gives the governor no steering; keep it only as a comparison baseline.
