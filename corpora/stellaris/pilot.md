...
> **DRAFT: to be finalized after the spike confirms the bridge.** Nothing below about the
> companion mod, the briefing format or the directive names has been exercised against the
> running game yet. What *is* verified (2026-09-25, user's PC): Stellaris "Cygnus v4.5.1
...
colony ships), and you steer it by choosing a **directive** — a stance that changes the native
AI's weights, the empire's policies and a few stance flags through a small companion mod.
...
2. **Urgent lines**, sent between briefings when `game.log` (companion-mod `log` effects, which
...
- **Issue one directive** via the bridge (the harness pauses, opens the console and fires the
  companion mod's event; see `strategy.md` § Governor directives). One directive at a time; it
