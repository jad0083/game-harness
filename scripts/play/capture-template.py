...
"""Capture a known-screen template from a live frame and measure how distinctive it is.
    ...
    scripts/play/capture-template.py FRAME X0 Y0 X1 Y1 NAME [--against FRAME ...]

FRAME is a 1568x882 screenshot (e.g. play/current_screen.jpg); the box is in its pixels.
Writes corpora/galciv4/templates/NAME.png and prints the normalized `template_roi` to paste
into a [screens.*] entry in corpora/galciv4/manifest.toml. With --against, prints the template
...
def main() -> None:
    ...
    out = ROOT / "corpora/galciv4/templates" / f"{a.name}.png"
    tpl.save(out)
